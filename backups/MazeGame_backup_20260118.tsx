...
import {
  Map as MapIcon, Shield, RefreshCw, Sparkles, AlertTriangle, Skull, Flame, CloudFog, Snowflake, HelpCircle, Settings, Trophy, Menu, X, Backpack, CheckSquare, Play, User, Globe
} from 'lucide-react';
...
import {
  LanguageCode, LANGUAGES, loadLanguagePreference, saveLanguagePreference, t, getLanguageInfo
} from '../utils/i18n';
...
const ShareButtons = ({ score, level, isWin }: { score: number; level: number; isWin: boolean }) => {
  const [copied, setCopied] = React.useState(false);
  const messages = isWin ? SHARE_MESSAGES_WIN : SHARE_MESSAGES_LOSE;
  const randomMessage = messages[Math.floor(Math.random() * messages.length)];

  const gameUrl = "https://nebula-maze.web.app";
  const shareText = isWin
    ? `I scored ${score.toLocaleString()} on Level ${level} of Nebula Maze Daily MazeTest! Can you beat me?`
    : `I scored ${score.toLocaleString()} on Nebula Maze Daily MazeTest! Think you can do better?`;
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-700">
      <p className="text-cyan-400 font-bold text-sm mb-3 text-center">{randomMessage}</p>
      <div className="flex justify-center gap-2 flex-wrap">
        {/* Facebook */}
        <a href={shareLinks.facebook} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 bg-blue-600 hover:bg-blue-500 rounded-full flex items-center justify-center transition-transform hover:scale-110">
          <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </a>
        {/* X/Twitter */}
        <a href={shareLinks.twitter} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 bg-black hover:bg-gray-800 rounded-full flex items-center justify-center transition-transform hover:scale-110">
          <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
        </a>
        {/* LinkedIn */}
        <a href={shareLinks.linkedin} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 bg-blue-700 hover:bg-blue-600 rounded-full flex items-center justify-center transition-transform hover:scale-110">
        ...
        <a href={shareLinks.whatsapp} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 bg-green-500 hover:bg-green-400 rounded-full flex items-center justify-center transition-transform hover:scale-110">
        ...
        <a href={shareLinks.telegram} target="_blank" rel="noopener noreferrer"
          className="w-10 h-10 bg-sky-500 hover:bg-sky-400 rounded-full flex items-center justify-center transition-transform hover:scale-110">
        ...
        <button onClick={copyToClipboard}
          className={`w-10 h-10 ${copied ? 'bg-green-600' : 'bg-slate-600 hover:bg-slate-500'} rounded-full flex items-center justify-center transition-transform hover:scale-110`}>
          {copied ? (
      ...
      {copied && <p className="text-green-400 text-xs text-center mt-2">Link copied!</p>}
    </div>
  );
};

// Landing page share messages
const LANDING_SHARE_MESSAGES = [
  "Share the Adventure!",
  "Challenge a Friend!",
...
// Share buttons for landing page (no score)
const LandingShareButtons = () => {
  const [copied, setCopied] = React.useState(false);
  const randomMessage = LANDING_SHARE_MESSAGES[Math.floor(Math.random() * LANDING_SHARE_MESSAGES.length)];

  const gameUrl = "https://nebula-maze.web.app";
  const shareText = "Challenge your brain with Nebula Maze - Daily MazeTest! Educational trivia meets maze adventure.";
...
// PSA Messages for sponsored section
const PSA_MESSAGES = [
  { text: "Drive Now, Drink at Home", icon: "🚗", color: "text-blue-700" },
  { text: "Drugs Are a Bullet in Slow Motion", icon: "💊", color: "text-red-600" },
  { text: "A Proud Partner Creates Half a Parent", icon: "👨‍👩‍👧", color: "text-purple-700" },
  { text: "Keep Families Whole", icon: "🏠", color: "text-green-700" },
  { text: "Post Your Homework Here = Zero Score", icon: "📝", color: "text-orange-600" },
  { text: "Your Mind is Your Greatest Asset", icon: "🧠", color: "text-cyan-700" },
  { text: "Stay Curious, Stay Learning", icon: "📚", color: "text-indigo-700" },
  { text: "Screen Time < Dream Time", icon: "😴", color: "text-slate-700" },
  { text: "Real Friends Don't Let Friends Text & Drive", icon: "📵", color: "text-red-700" },
  { text: "Be the Change You Want to See", icon: "✨", color: "text-amber-600" },
];
...
const Launcher = ({
  ...
  // Dynamic UTC date state
  const [utcDate, setUtcDate] = React.useState('');

  // Rotating PSA message
  const [psaIndex, setPsaIndex] = React.useState(() => Math.floor(Math.random() * PSA_MESSAGES.length));

  React.useEffect(() => {
    const interval = setInterval(() => {
      setPsaIndex(prev => (prev + 1) % PSA_MESSAGES.length);
  ...
  React.useEffect(() => {
    const updateDate = () => {
      ...
      setUtcDate(`${dayName} ${ordinal(day)} ${month}, ${year}`);
    };
...
export const MazeGame: React.FC = () => {
  ...
  // Save language preference and reload questions when language changes
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Prevent arrow keys from causing text selection/scrolling
          ...
          <div className="grid grid-cols-4 gap-1 mb-1.5">
            {bag.map((item, i) => (
              <button
                ...
                className="w-10 h-10 bg-slate-700 hover:bg-slate-600 rounded flex flex-col items-center justify-center transition-colors border border-slate-600"
                title={ITEMS_CONFIG[item]?.name}
              >
                <span className="text-sm">{ITEMS_CONFIG[item]?.char}</span>
                <span className="text-[5px] text-slate-400 truncate w-full text-center leading-none">{ITEMS_CONFIG[item]?.name}</span>
              </button>
            ))}
          </div>
         ...
         <div className="flex-1 flex flex-col items-center justify-start py-1 min-h-0 overflow-y-auto"
          ...
          <div className="flex flex-col md:hidden items-center justify-start gap-2 flex-shrink-0 px-1 py-1">
            ...
            <div
              ...
              style={{
                width: 'min(98vw, calc(100vh - 200px))',
                height: 'min(98vw, calc(100vh - 200px))',
                maxWidth: '1200px',
                maxHeight: '1200px',
                aspectRatio: '1 / 1',
            ...
            <div className="flex flex-col gap-3 items-center justify-center flex-shrink-0 pb-3">
              {/* Up Arrow */}
              <button
                onClick={() => handleManualMove(0, -1)}
                className="w-24 h-24 bg-slate-800/90 border-2 border-slate-600 rounded-xl hover:bg-slate-700 active:bg-slate-600 flex items-center justify-center text-4xl shadow-lg transition-all active:scale-95"
                aria-label="Move Up"
              >
                ⬆️
              </button>
    ...
    `}</style>
        ...
        {modal.isOpen && modal.type !== 'inventory' && modal.type !== 'question' && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-sm shadow-2xl p-6 animate-in zoom-in max-h-[90vh] overflow-y-auto">
              ...
              {/* Share Buttons for Level Complete and Game Over */}
              {(modal.type === 'levelComplete' || modal.type === 'gameOver') && (
                <ShareButtons score={score} level={level} isWin={modal.type === 'levelComplete'} />
              )}
            </div>
          </div>
        )}
        ...
        <div className="hidden md:flex w-[80px] bg-slate-900 border-l border-slate-800 flex-col items-center justify-center p-1 gap-2">
           <div className="text-[9px] text-slate-500 font-bold uppercase text-center leading-tight" style={{writingMode: 'vertical-rl', textOrientation: 'mixed'}}>Advertisement Here</div>
           <div className="w-full h-[200px] bg-white/5 rounded flex items-center justify-center text-slate-500 text-[9px] text-center px-1">Advertisement Here</div>
        </div>
